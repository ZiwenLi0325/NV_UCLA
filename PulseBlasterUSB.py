...
class PulseBlasterUSB():
    def __init__(self, board_number=0, **kwargs):
        self.Initialize(board_number)
    ...
    def Remove_Buffer_Sequence(self, index):
    ...
    def Loop(self, inst_list, loops):
        new_inst_list = [[], [], [], []]
        if inst_list[0][0] == inst_list[0][-1] and len(inst_list[0]) != 1:
