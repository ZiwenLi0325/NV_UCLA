...
class PulseBlasterESRPRO(Instrument):
    """
    This is the qcodes driver for the PulseBlasterUSB.
    ...
    def flush_channel_buffer(self, repeat : bool=True, contiuous : bool=True):
        ...
        time=[0]
        flags=[int(0)]
        for i in range(0,N): #i is channel of interest
            M=len(d_table[i]) #M is number of instructions in channel i
            k=0 #Tracking number for
            for j in range(0,M): #Loops for the jth instruction over all instructions in channel i
                K=len(time) #Tracks how many
                while time[k] < t_table[i][j]: #NOTE: Only accesses start times, as we never get to last entry of t_table[i]
                    k+=1
                    if k==K:
                        break
                if k==K:
                    time.append(t_table[i][j])
                    flags.append(flags[k-1] & ~(1 << i)) #~(1 << i) = -(2**(i-1)+1) through bitwise operations. & compares bits. If paired then 1, if not then 0.
